...
class BAOPowerModel(object):
    ...
    def __init__(self, n_gauss=30, no_wiggle=False, inflate_error=False):
        ...
        for index_z in range(self.n_bin):
            if no_wiggle:
                data = np.loadtxt(os.path.join(self.data_directory, self.theory_pk_file_nw[index_z]))
            else:
        ...
        self.k_vals = self.k_vals_theory
